...
from backend.models import  Group, group_members,Userfrom flask_socketio import emit
...
def chat_routes(chats_bp, db, socketio):
    ...
    def request_to_join(group_id):
        ...
        existing_request = GroupJoinRequest.query.filter_by(group_id=group_id, user_id=current_user.id).first()
        if existing_request:
            return jsonify({'error': 'Request already sent or already a member'}), 400

        # Create a join request
        join_request = GroupJoinRequest(group_id=group_id, user_id=current_user.id)
        db.session.add(join_request)
        db.session.commit()

        # Notify the group creator in real-time
        group_creator = User.query.get(group.created_by)
        notification_message = f"{current_user.name} has requested to join the group '{group.name}'."
        emit(
            'group_request_notification',
            {'message': notification_message, 'group_id': group_id, 'user_id': current_user.id},
            room=f'user_{group_creator.id}',  # Send to the specific user room
